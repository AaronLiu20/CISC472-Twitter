...
let UpdateData = (placement, id, content) => {
  firebase.database().ref(placement).child(id).update(content);
}
    ...
    firebase.auth().signInWithPopup(provider).then((result) => {
      ...
      firebase.database().ref( UserDBR).once("value").then( (ss) => {
        if(!ss.child(result.user.uid).exists()){
          ...
          updateData(UsernameDBR, newHandle, newUsername);
        }
      });
...
firebase.auth().onAuthStateChanged(user=>{
  ...
  if(user){
    ...
    TweetDBR.on("child_added" , (ss) =>{
      ...
      firebase.database().ref(UserDBR).child(ss.val().uid).get().then( (result) =>{
        let sameUser = (tempTweet.uid === user.uid);
        renderTweets(tempTweet, ss.key, result.val(), "insertTweets", sameUser);
        if(sameUser){
    ...
    document.getElementById("Writing-Tweets").onclick = function(event){
      alert("You need to log in to create a tweet");
    }
